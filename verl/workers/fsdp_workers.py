...
class ActorRolloutRefWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(
                ...
                lr_scheduler=None,  # Actor doesn't typically use lr_scheduler in PPO
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        self.checkpoint_manager.save_checkpoint(
            ...
            hdfs_path=hdfs_path,
            global_step=0,  # We don't track global step at worker level
            max_ckpt_to_keep=None,  # Cleanup is handled at trainer level
        )

        # Offload optimizer back if needed
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
    ...
    def load_checkpoint(
        ...
        # Load checkpoint using the manager
        ...
        if self.rank == 0:
            print(f"Actor checkpoint loaded successfully from {checkpoint_path}")
...
class CriticWorker(Worker):
    ...
    def init_model(self):
        ...
        self.checkpoint_manager = FSDPCheckpointManager(
            ...
            lr_scheduler=None,  # Critic doesn't typically use lr_scheduler in PPO
            processing_class=self.tokenizer,
            checkpoint_contents=["model", "hf_model", "optimizer", "extra"],
        )
        torch.cuda.empty_cache()
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(
        self, checkpoint_path, hdfs_path=None, del_local_after_load=False
    ):
        ...
        if self.rank == 0:
            print(f"Critic checkpoint loaded successfully from {checkpoint_path}")
